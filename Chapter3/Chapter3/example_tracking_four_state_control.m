...
for k = 1:length(t)
    ...
    eX = e(1); eY = e(2); % Two errors (due to distance)
    eS = sin(e(3)); eCos = cos(e(3)); eC = eCos - 1; % Orientation errors
    ...
    a = 10;     % Experiment with this control design parameter
    n = 2;      % Experiment with this parameter (integer)
    Kx = 2*zeta*sqrt(wRef^2+g*vRef^2);
    Ks = Kx;
    K = g;
    % Gains Kx and Ks could also be constant.
    ...
    w = wRef + K*vRef*eY*(1+eC/a)^2 + Ks*eS*(1+eC/a)^(2*n);
