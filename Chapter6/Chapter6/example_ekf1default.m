...
est = OWmr(); est.showWmr('r:'); est.showDistance('m:');
              est.showAngle('r:'); est.showPath('r.-'); est.setPose(x);
wmr = OWmr(); wmr.showWmr('y-'); wmr.showDistance('m-');
              wmr.showAngle('r-'); wmr.showPath('b--'); wmr.setPose(xTrue);
...
for k = 1:N
    ...
    % Simulation of the true noisy measurements
    zTrue = [sqrt(xTrue(1)^2 + xTrue(2)^2 ); ...
             xTrue(3)] + sqrt(R)*randn(2, 1)*enableNoise;
    zTrue(1) = abs(zTrue(1));
